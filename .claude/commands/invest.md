...
- prompt: 以下の内容を含めてください
  ...
  - 以下の分析指示:
    ```
    以下の市場データとポートフォリオデータを分析し、あなたの専門的視点から投資判断に役立つ分析を提供してください。

    ## 市場データ (tmp/market.json)
    [tmp/market.json の全内容]

    ## ポートフォリオデータ (tmp/portfolio.json)
    [tmp/portfolio.json の全内容]
...

    {
      ...
      "risks": ["リスク1", "リスク2"],
      "picks": [
        {"ticker": "AAPL", "direction": "強気", "rationale": "推奨理由（100文字以内）"}
      ],
...
- prompt: 以下の内容を含めてください
  ...
  - 以下の分析指示:
    ```
    以下の市場データとニュースデータを分析し、あなたの専門的視点から投資判断に役立つ分析を提供してください。
    ...
    注意: picksのtickerは必ず英数字ティッカー形式（例: AAPL, 7203.T）で記入してください。
    macroはセクターレベルでの推奨が主体なので、個別銘柄picksは0件でも構いません。
    ```
...
- prompt: 以下の内容を含めてください
  ...
  - 以下の分析指示:
    ```
    以下の市場データ・ニュースデータ・ポートフォリオデータを分析し、あなたの専門的視点から投資判断に役立つ分析を提供してください。
...

    {
      ...
      "risks": ["リスク1", "リスク2"],
      "picks": [
        {"ticker": "AAPL", "direction": "弱気", "rationale": "警戒理由（100文字以内）"}
      ],
      "sectorView": "セクター見通し（1-2文）"
    }
