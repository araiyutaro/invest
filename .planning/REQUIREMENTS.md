...
- [x] **CURA-06**: 各グループ内で記事が重要度順に配列される
- [x] **CURA-07**: 各記事に重要度バッジ（High / Medium / Low）が表示される（重要度順配列と同一のスコアから導出）
- [x] **CURA-08**: 各記事に関連ティッカータグが表示される（既存tickerフィールド + キュレーション時のタイトル/サマリーからの抽出）
- [x] **CURA-09**: ページ冒頭に「今日の市場を動かすもの」リード文（2〜3文の総括パラグラフ）が表示される
...
| CURA-06 | Phase 16 | Complete |
| CURA-07 | Phase 16 | Complete |
